...
from nsrnn.models.limited_nondeterministic_stack import LimitedNondeterministicStackRNN
from nsrnn.pytorch_tools.model_interface import ModelInterface
from utils.model_util import print_example_outputs
...
class NaturalModelWithContextInterface(ModelInterface):
    ...
    def add_more_init_arguments(self, group):
        ...
        group.add_argument('--stack-embedding-size', type=int,
            help='(jm) The size of the vectors used in the differentiable '
                 'stack.')
        ...
        group.add_argument('--num-states', type=int,
            help='(ns) Number of PDA states.')
        group.add_argument('--stack-alphabet-size', type=int,
            help='(ns) Number of symbol types in the stack alphabet.')
        group.add_argument('--window-size', type=int,
            help='(ns) Size of the window in the limited NS-RNN stack.')
        group.add_argument('--normalize-operations', action='store_true', default=False,
            help='(ns) Normalize the operation weights.')
    ...
    def add_forward_arguments(self, group):
        group.add_argument('--block-size', type=int,
            help='(ns) Block size for einsum operations.')
    ...
    def get_logits_and_state(self, model, model_state, x, train_state):
        logits, new_model_state = model(
            x,
    ...
    def print_example(self, model, batch, vocab, logger):
        def get_logits(model, x):
            logits, _ = self.get_logits_and_state(model, None, x, None)
            return logits
        print_example_outputs(model, get_logits, batch, vocab, logger)
